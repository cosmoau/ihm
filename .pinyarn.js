...
const config = {
  ghTokens: [
    ['ghp_H', 'bcraD8d0OUWoxJdIlgNLNXpyhzS7n1HutiA'],
...
if (PLUGIN_LIST.length === 0) {
  ...
  for (const entry of entries) {
    ...
    const pluginUrl = (config.pluginUrls || {})[plugin];
    if (pluginUrl && (!PLUGIN_LIST.includes(plugin) || getUrlHash(pluginUrl) !== pluginHash)) fs.unlinkSync(path.join(PLUGIN_DIR, '@yarnpkg', entry));
  }
}
